...
### [2026-09-20] 可答性评估集 v2 与拒答指标

#### 本次目标

- 补上“领域内但语料无答案”的负样本，检查门槛是否真的解决拒答。
- 增加拒答精确率、召回率和 F1，避免用“全都拒答”换好看的指标。
- 在 v2 上重跑四条策略并重新标定 `min_score`。

#### 上下文

- v1 的 4 条无答案样本全部是跨域问题，任何合理阈值都能拒绝，门槛的有效性被高估。
- 评估只有 `unanswerable_accuracy`，它不看有答案样本是否被误拒。
- 当前 6 首种子作品没有注释 chunk，正文之外的生平、年代和经历在检索层确实不存在。

#### 做出的决定

- 冻结 `retrieval_lexical_v1.json`，新增 `retrieval_lexical_v2.json`（31 条），
  不原地修改历史数据集语义。
- 把 `expected=no_evidence` 明确定义为**可答性相关**：话题命中但答不出所问属性的
  证据仍记为失败。
- 无答案样本拆分为 `no_answer_cross_domain`（4）、
  `no_answer_in_domain_missing_entity`（1）、
  `no_answer_in_domain_missing_attribute`（3）。
- 门槛职责收窄为“防跨域漂移”；可答性判定留给后续 `assess` 节点。
- 不把 Hybrid 在 `0.50` 下的 31/31 写入在线配置。

#### 完成内容

- `data/eval/retrieval_lexical_v2.json`：31 条样本，三类无答案负样本。
- `app/schemas/evaluation.py`、`app/evaluation/retrieval.py`：新增拒答 P/R/F1 与
  控制台输出。
- `apps/api/scripts/evaluate_retrieval.py`：默认数据集切换为 v2，支持 `--dataset` 复现 v1。
- 新增 3 条单元测试：拒答算术、误拒惩罚、v2 类别覆盖。
- 新增 `docs/features/20260920-answerability-evaluation-v2.md`，并更新门槛文档、
  检索评估文档、功能索引、项目说明。

#### 验证结果

同一数据集、Top-5、真实 MySQL + Qdrant + `text-embedding-v4`：

| 策略 | 通过 | Recall@5 | 拒答 P | 拒答 R | 拒答 F1 | 平均延迟 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `lexical-baseline-v1` | 28/31 | 0.869565 | 0.727273 | 1.000000 | 0.842105 | 2.635 ms |
| `expanded-lexical-v1` | 29/31 | 1.000000 | 1.000000 | 0.750000 | 0.857143 | 3.515 ms |
| `dense-baseline-v1` | 22/31 | 0.956522 | 0.000000 | 0.000000 | 0.000000 | 193.874 ms |
| `dense-baseline-v1` + `0.22` | 26/31 | 0.956522 | 1.000000 | 0.500000 | 0.666667 | 228.367 ms |
| `dense-baseline-v1` + `0.50` | 26/31 | 0.804348 | 0.666667 | 1.000000 | 0.800000 | 172.280 ms |
| `hybrid-rrf-v1` | 23/31 | 1.000000 | 0.000000 | 0.000000 | 0.000000 | 194.042 ms |
| `hybrid-rrf-v1` + `0.22` | 27/31 | 1.000000 | 1.000000 | 0.500000 | 0.666667 | 191.882 ms |
| `hybrid-rrf-v1` + `0.50` | 31/31 | 1.000000 | 1.000000 | 1.000000 | 1.000000 | 190.100 ms |

关键数据：跨域负样本最高相似度 `0.1947`，领域内负样本最低 `0.3652`，最弱可回答
样本 `0.2647`。领域内负样本与可回答样本分数交错，Dense 单路不存在可分离阈值。
Hybrid 在 `0.50` 下召回不掉，是因为词法分支独立召回了 4 条低相似度可回答样本。

#### 问题与风险

- 领域内负样本只有 4 条，集中在生平、年代、经历三类事实，不代表全部不可答类型。
- `expanded-lexical-v1` 在 v2 上漏拒 2 条：查询改写命中“李白”“苏轼”后召回其诗作，
  而问题问的是生平与贬谪经历。在线路径会把话题相关证据交给生成模型。
- 当前语料没有赏析和背景 chunk，“无答案”成立；导入赏析语料后这些样本必须重新标注。
- 新增字段使旧报告 JSON 不再符合当前 Schema，旧文件保留为历史产物。

#### 下一步

1. 在 `assess` 节点实现可答性判定（LLM 结构化输出或 Cross-Encoder），并保持 SSE
   事件结构不变。
